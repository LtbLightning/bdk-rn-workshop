...
let signOptions = new SignOptions(
  false,
  false,
  152,
  false,
...
const Ffi = () => {
  ...
  const [blockchain1, setBlockchain1] =
    useState<Blockchain>(blockchainInstance);
  ...
  const walletInit = async () => {
    try {
      ...
      const secretKeyObj = await new DescriptorSecretKey().create(
        network,
        mnemonicObj,
      );
      const descriptorObj = await new Descriptor().newBip84(
        secretKeyObj,
      ...
      const w = await walletInstance.create(
        descriptorObj,
        null,
        network,
        dbConfig,
  ...
  const getAddress = async () => {
    ...
    console.log(
      'Internal Address::',
      await internalAddress?.address.asString(),
    );
  ...
  const sendBit = async (one = true) => {
    ...
    if (one) {
      ...
      let address = await new Address().create(
        'tb1qccmtnhczmv3a6k4mtq8twm7ltj3e32qsntmamv',
      );
      let script = await address.scriptPubKey();
      let txBuilder = await new TxBuilder().create();
  ...
  const extraTxMethods = async () => {
    ...
    let amount = 999;
    let script = await (
      await new Address().create(addressHash, networkString)
    ).scriptPubKey();
    let script1 = await (
      await new Address().create(
        'tb1qvdgr0mhn354zzc5q6ex0p6stkuj4qcjl640ju2',
        networkString,
      )
    ).scriptPubKey();
    ...
    const scriptAmounts: Array<ScriptAmount> = [
      new ScriptAmount(script, amount),
      new ScriptAmount(script1, amount),
    ];
  ...
  const descriptors = async () => {
    ...
    let descObject = await new Descriptor().create(
      `wpkh(${xprvWallet})`,
      networkString,
    );
    const bip44 = await new Descriptor().newBip44(
      secretKey,
      KeychainKind.External,
      networkString,
    );
    const bip49 = await new Descriptor().newBip49(
      secretKey,
      KeychainKind.External,
      networkString,
    );
    const bip84 = await new Descriptor().newBip84(
      secretKey,
      KeychainKind.External,
      networkString,
    );
    const bip86 = await new Descriptor().newBip86(
      secretKey,
      KeychainKind.External,
      networkString,
    );
    ...
    console.log(
      '86--->',
      bip86,
      await bip86.asString(),
      await bip86.asStringPrivate(),
    );
  };
  ...
  const publicDescriptors = async () => {
    try {
      ...
      const bip86 = await new Descriptor().newBip86Public(
        pubKey,
        fingerprint,
        KeychainKind.External,
        networkString,
  ...
  const bumpTx = async (
    tid = 'da6e62280471c504dbf92baf78af5bfdde3e6d457ea55cf43c490f091740c898',
  ) => {
    let addr = await new Address().create(
      'tb1qccmtnhczmv3a6k4mtq8twm7ltj3e32qsntmamv',
      networkString,
    );
    ...
    console.log(
      'RBF Sequence::',
      await bumpTxBuilder.enableRbfWithSequence(15),
    );
    ...
    <SafeAreaView
      ...
      <ScrollView
        ...
        <View>
          ...
          <Button
            transparent
            title="Descriptor Secret"
            onPress={descriptorSecret}
          />
          <Button
            transparent
            title="Descriptor Public"
            onPress={descriptorPublic}
          />
          ...
          {wallet?.isInit && (
            <Fragment>
              ...
              <Button
                transparent
                title="Address Extras"
                onPress={() => addressExtras()}
              />
            </Fragment>
          )}
          <Button
